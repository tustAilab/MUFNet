...
def run(modelname="r2plus1d_18",
    ...
    ds = echonet.datasets.Echo(split="test", **kwargs, clips="all")
    dataloader = torch.utils.data.DataLoader(
        ds, batch_size=1, num_workers=num_workers, shuffle=False, pin_memory=(device.type == "cuda"))

    model.eval()

    loss, yhat, y = echonet.utils.video.run_epoch(model, dataloader, False, None, device, save_all=True, block_size=1)


    # Write full performance to file


    # with open(os.path.join(output, "{}_predictions.csv".format("test")), "w") as g:
    ...
    # plt.tight_layout()
    # plt.savefig(os.path.join(output, "{}_scatter.pdf".format("test")))
    # plt.close(fig)
    #
    # # Plot AUROC
    # fig = plt.figure(figsize=(3, 3))
    ...
    # plt.tight_layout()
    # plt.savefig(os.path.join(output, "{}_roc.pdf".format("test")))
    # plt.close(fig)
