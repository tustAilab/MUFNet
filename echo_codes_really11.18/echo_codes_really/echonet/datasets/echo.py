...
class Echo(torch.utils.data.Dataset):
    ...
    def __init__(self, root=None,
        ...
        if root is None:
            root =   r'/media/E/mjp/echonet-dynamic'#echonet.config.DATA_DIR
    ...
    def __getitem__(self, index):
        ...
        for t in self.target_type:
            ...
            if t == "Filename":
                ...
                coo.append(co)
                #zero=np.zeros((112,112,3),dtype=np.uint8)
                # for coord in coo:
                #     zero[coord[0],coord[1]]=[255,0,0]
                # plt.imshow(zero)
                # plt.show()
                target.append(mask)
            else:
...
# print(mean)
    ...
    train_dataloader = torch.utils.data.DataLoader(
            train_dataset, batch_size=20, num_workers=4, shuffle=True, pin_memory=(device.type == "cuda"), drop_last=False)
    # dataset = echonet.datasets.Echo(split="val",
    ...
    for i, (imgs, (large_Index, small_Index, large_trace, small_trace)) in enumerate(train_dataloader):
        # f = iter(train_dataloader).to(device)
