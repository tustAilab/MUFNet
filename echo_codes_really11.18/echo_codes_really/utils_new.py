...
def pm_loss(alpha, x, y):
    ...
    #oseg = F.l1_loss(pred, trace)
    pm_loss = (alpha*(1-pytorch_msssim.ssim(x,y))/2 + (1-alpha) * F.l1_loss(x, y))
    #pm_loss_2=(alpha*(1-pytorch_msssim.ssim(wseg,seg))/2 + (1-alpha) * F.l1_loss(wseg, seg))
    # oseg = torch.abs((pred_seg1-warped_seg1))
    # pm_loss = g
        # pm_loss.append(p_loss)
    # pm_loss = torch.sum(pm_loss)
    return pm_loss
def DSSIM(x,y):
...
def smoothness_loss_mjp(M, I):
    # 计算M和I的梯度
    grad_M_x = gradient_x(M)
    grad_M_y = gradient_y(M)
    grad_I_x = gradient_x(I)
    grad_I_y = gradient_y(I)

    # 计算梯度的绝对值
    abs_grad_M_x = torch.abs(grad_M_x)
    abs_grad_M_y = torch.abs(grad_M_y)
    abs_grad_I_x = torch.abs(grad_I_x)
    abs_grad_I_y = torch.abs(grad_I_y)

    # 计算指数衰减因子
    exp_factor_x = torch.exp(-abs_grad_I_x)
    exp_factor_y = torch.exp(-abs_grad_I_y)

    # 确保维度匹配
    if abs_grad_M_x.shape != exp_factor_x.shape:
        # 调整exp_factor_x的维度以匹配abs_grad_M_x
        exp_factor_x = exp_factor_x.unsqueeze(1)  # 假设需要在通道维度上扩展
    if abs_grad_M_y.shape != exp_factor_y.shape:
        # 调整exp_factor_y的维度以匹配abs_grad_M_y
        exp_factor_y = exp_factor_y.unsqueeze(1)  # 假设需要在通道维度上扩展

    # 计算损失
    loss_x = abs_grad_M_x * exp_factor_x
    loss_y = abs_grad_M_y * exp_factor_y
    loss = loss_x + loss_y

    # 对所有元素求和并除以元素总数
    return torch.sum(loss) / (M.shape[0] * M.shape[2] * M.shape[3])
def temporal_consistency_loss(warped_segs, seg):
    ...
    # warped_segs = (warped_segs > 0).float()
    # seg = (seg > 0).float()
...
def unsup_loss(seg1_pred, warped_segs1, pred_flows, warped_imgs1, frame1, seg2_pred,
               large_Index,small_Index,large_trace,small_trace):
    ...
    dice_sm = 0
    dice_lg = 0
    # for i in range (len(warped_imgs1)):

    #bce += 5 * photometric_loss(warped_imgs1[0], frame1) #光度损失
    #bce +=5*pm_loss(0.85,warped_imgs1[0], frame1)
    smooth += 0.2 *flow_smooth_loss(pred_flows[0],frame1)
    bce += 5* pm_loss(0.85, warped_imgs1[0], frame1)
    tc_loss += 0.4 * temporal_consistency_loss(warped_segs1[0], seg1_pred)
    # print(seg1_pred.size())


    #算bce和dice loss
    seg_loss += torch.nn.functional.binary_cross_entropy_with_logits(seg2_pred[large_Index][0], large_trace,reduction="mean")
    pred_lg = torch.sigmoid(seg2_pred[large_Index][0])
    pred_lg = ( pred_lg > 0.5).float()
    dice_lg += Dice_loss(pred_lg, large_trace).item()

    # smooth += smooth_loss(pred_flows[0], frame1) * (1 + torch.abs(pred_lg - large_trace))

    seg_loss += torch.nn.functional.binary_cross_entropy_with_logits(seg2_pred[small_Index][0], small_trace,reduction="mean")
    pred_sm = torch.sigmoid(seg2_pred[small_Index][0])
    pred_sm = (pred_sm > 0.5).float()
    dice_sm += Dice_loss(pred_sm, small_trace).item()
    #bce += 5 * pm_loss(0.85, warped_imgs1[0], frame1, pred_sm, small_trace)
    #smooth += smooth_loss(pred_flows[0], frame1) * (1 + torch.abs(pred_sm-small_trace))


    loss = bce + smooth + seg_loss + tc_loss + dice_lg +dice_sm

    return loss, bce, smooth, seg_loss, tc_loss, dice_sm, dice_lg